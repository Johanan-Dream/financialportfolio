...
public class SavingBankjCalculatorPage extends Page {
	...
	public SavingBankjCalculatorPage(PortfolioMain portfolioMain) {
		super(portfolioMain);
		...
		table.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				if (table.getSelectedColumn() == 3) {// 테이블의 컬럼을 선택했을때 정보 반영
		...
		bt_add.addActionListener(new ActionListener() {// if문 손보기--입력된 내용이 없으면 버튼 비활성화, enable
			public void actionPerformed(ActionEvent e) {
				if (t_totalAmount.getText().equals("") || t_ibtd.getText().equals("")
						|| t_taxAmount.getText().equals("") || t_atra.getText().equals("")) {
					JOptionPane.showMessageDialog(portfolioMain, "상품정보를 확인해주세요.", "안내", JOptionPane.INFORMATION_MESSAGE);
					// bt_add.setEnabled(false);
				} else {
					// 상품등록이 성공할때(상품이 몇개 추가되었습니다.)
					// 버튼 누를때 정보 DTO에 담기
		...
		bt_cal.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (t_bankname.getText().equals("") || t_productType.getText().equals("")
						|| t_productName.getText().equals("") || t_amount.getText().equals("")) {
	...
	public void depositCal() {// 예금 계산식
		if(t_irtype.getText().equals("단리")) {
			...
			double tax = 15.4 / 100;// 우선 고정 나중에 변동될 수 있음
			// 예금계산식
	...
	public void productList() {// 페이지로 오는 순간 상품 보이기
		model.list = portfolioMain.productNameDAO.savingBankSelectAll();
		table.updateUI();
	}
	...
	public void search() {
		if(box_search1.getSelectedIndex()==0 & box_search2.getSelectedIndex()==0) {//아무것도 선택되지 않았을때
			JOptionPane.showMessageDialog(portfolioMain, "카테고리를 선택해주세요", "안내", JOptionPane.INFORMATION_MESSAGE);		
		}else {
			...
			model.list=portfolioMain.productNameDAO.savingBankSelect(ptype, period, content);
			table.updateUI();
			}
		}
}
